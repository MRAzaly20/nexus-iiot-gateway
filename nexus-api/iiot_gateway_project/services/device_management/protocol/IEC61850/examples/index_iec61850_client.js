    ...
    if (event === 'conn' && data.event === 'opened') {
        ...
        client.browseDataModel()
            .then(dataModel => {
                ...
                console.log('And now we try do control operation!!!!!!!!!!!!!!!...');
                client.controlObject("simpleIOGenericIO/GGIO1.SPCSO2", true);
                ...
                dataRefs.forEach(ref =>
                    console.log("This data >", client.readData(ref))
                );
...
async function main() {
    ...
    try {
        ...
        await client.connect({
            ...
            port: 102,
            clientID: 'mms_client1',
            reconnectDelay: 2 // auto-reconnect setiap 2 detik jika gagal
        });

        console.log('✅ Client connected and running indefinitely.');
        console.log('📥 Listening for reports, data, and control events...');
        console.log('🛑 Press Ctrl+C to exit gracefully.');

        // Opsional: Periodic heartbeat
        setInterval(() => {
            const status = client.getStatus();
            console.log(`[💓 HEARTBEAT] Status: ${status}`);
        }, 30000);

        // Biarkan hidup selamanya — event handler akan menangani semuanya
        // Tidak perlu loop, tidak perlu sleep — Node.js event loop akan menjaga proses tetap hidup
...
process.on('SIGINT', async () => {
    console.log('\n🛑 Received SIGINT. Closing client gracefully...');
    await client.close().catch(e => console.error('CloseOperation error:', e.message));
    console.log('✅ Client closed. Goodbye!');
    process.exit(0);
});