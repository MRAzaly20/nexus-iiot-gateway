...
std::string NapiValueToJsonString(Napi::Env env, Napi::Value value);

std::string NapiObjectToJsonString(Napi::Env env, Napi::Object obj)
{
    std::string json = "{";
    ...
    for (uint32_t i = 0; i < keys.Length(); i++)
    {
        ...
        json += "\"" + EscapeJsonString(keyStr) + "\":";
        json += NapiValueToJsonString(env, val);
    }
    json += "}";
    return json;
}

std::string NapiArrayToJsonString(Napi::Env env, Napi::Array arr)
{
    std::string json = "[";
...
std::string NapiValueToJsonString(Napi::Env env, Napi::Value value)
{
    ...
    else if (value.IsObject())
    {
        ...
        return NapiObjectToJsonString(env, obj);
    }
...
{
    Napi::Function func = DefineClass(env, "MmsClient", {InstanceMethod("connect", &MmsClient::Connect), InstanceMethod("readData", &MmsClient::ReadData), InstanceMethod("controlObject", &MmsClient::ControlObject), InstanceMethod("close", &MmsClient::Close), InstanceMethod("getStatus", &MmsClient::GetStatus), InstanceMethod("getLogicalDevices", &MmsClient::GetLogicalDevices), InstanceMethod("readDataSetValues", &MmsClient::ReadDataSetValues), InstanceMethod("createDataSet", &MmsClient::CreateDataSet), InstanceMethod("deleteDataSet", &MmsClient::DeleteDataSet), InstanceMethod("getDataSetDirectory", &MmsClient::GetDataSetDirectory), InstanceMethod("browseDataModel", &MmsClient::BrowseDataModel), InstanceMethod("enableReporting", &MmsClient::EnableReporting), InstanceMethod("disableReporting", &MmsClient::DisableReporting)});
...
Napi::Value MmsClient::BrowseDataModel(const Napi::CallbackInfo &info)
{
    ...
    try
    {
        ...
        // 👇 KONVERSI KE STRING SEBELUM LAMBDA
        std::string jsonArray = NapiArrayToJsonString(env, resultArray);

        // 👇 CEK isClosing_ SEBELUM MEMANGGIL CALLBACK
        if (!isClosing_)
        {
            tsfn_.NonBlockingCall([this, jsonArray](Napi::Env env, Napi::Function jsCallback)
                                  {
                if (this->isClosing_) {
                    printf("BrowseDataModel: Skipping callback — client is closing, clientID: %s\n", this->clientID_.c_str());
                    return;
                }
                Napi::Object eventObj = Napi::Object::New(env);
                eventObj.Set("clientID", Napi::String::New(env, this->clientID_.c_str()));
                eventObj.Set("type", Napi::String::New(env, "data"));
                eventObj.Set("event", Napi::String::New(env, "dataModel"));
                eventObj.Set("dataModelJson", Napi::String::New(env, jsonArray));
                jsCallback.Call({Napi::String::New(env, "data"), eventObj}); });
        }
        else
        {
            printf("BrowseDataModel: Not emitting event — client is closing, clientID: %s\n", clientID_.c_str());
        }
    ...
    try
    {
        ...
        for (auto tryFc : fcs)
        {
            ...
            if (error == IED_ERROR_OK && value != nullptr)
            {
                ...
                // 👇 Tambahkan ini: cetak nilai sesuai tipe
                char valueStr[256] = {0}; // buffer untuk string nilai

                switch (MmsValue_getType(value))
                {
                case MMS_BOOLEAN:
                    snprintf(valueStr, sizeof(valueStr), "BOOLEAN: %s",
                             MmsValue_getBoolean(value) ? "true" : "false");
                    break;

                case MMS_INTEGER:
                case MMS_UNSIGNED:
                    snprintf(valueStr, sizeof(valueStr), "INTEGER: %d",
                             MmsValue_toInt32(value));
                    break;

                case MMS_FLOAT:
                    snprintf(valueStr, sizeof(valueStr), "FLOAT: %f",
                             MmsValue_toFloat(value));
                    break;

                case MMS_VISIBLE_STRING:
                    snprintf(valueStr, sizeof(valueStr), "STRING: \"%s\"",
                             MmsValue_toString(value));
                    break;

                case MMS_STRUCTURE:
                    snprintf(valueStr, sizeof(valueStr), "STRUCTURE (complex type)");
                    break;

                default:
                    snprintf(valueStr, sizeof(valueStr), "UNSUPPORTED TYPE: %d",
                             MmsValue_getType(value));
                    break;
                }
                //add dataref on return
                return Napi::String::New(env, std::string(dataRef + ": " + valueStr));                //return Napi::String::New(env, "" + std::string(valueStr) + "\n"); // ← CETAK NILAI NYATA
        ...
        if (error != IED_ERROR_OK || value == nullptr)
        {
            ...
            case IED_ERROR_TYPE_INCONSISTENT:
                errorMsg = "Type inconsistent";
                break;
            case IED_ERROR_OBJECT_ACCESS_UNSUPPORTED:
                errorMsg = "Object access unsupported";
                break;
            default:
        ...
        convertMmsValue = [&](MmsValue *val, const std::string &attrName) -> ResultData
        {
            ...
            case MMS_INTEGER:
            {
                ...
                if (attrName == "ctlModel")
                {
                    ...
                    {
                    case 0:
                        data.stringValue = "status-only";
                        break;
                    case 1:
                        data.stringValue = "direct-with-normal-security";
                        break;
                    case 2:
                        data.stringValue = "sbo-with-normal-security";
                        break;
                    case 3:
                        data.stringValue = "direct-with-enhanced-security";
                        break;
                    case 4:
                        data.stringValue = "sbo-with-enhanced-security";
                        break;
                    default:
                        data.stringValue = "unknown(" + std::to_string(data.intValue) + ")";
                    }
                }
                break;
            }
            ...
            case MMS_BIT_STRING:
            {
                ...
                if (attrName == "q")
                {
                    ...
                    if (quality & QUALITY_VALIDITY_INVALID)
                        qualityStr += "Invalid|";
                    if (quality & QUALITY_VALIDITY_QUESTIONABLE)
                        qualityStr += "Questionable|";
                    if (qualityStr.empty())
                        qualityStr = "Good";
                    else
                        qualityStr.pop_back();
        ...
        auto mmsValueToString = [](MmsValue *value) -> std::string
        {
            ...
            case MMS_INTEGER:
            {
                ...
                if (MmsValue_getType(value) == MMS_INTEGER && intVal >= 0 && intVal <= 4)
                {
                    ...
                    default:
                        return "unknown(" + std::to_string(intVal) + ")";
                    }
                }
        ...
        auto readAttributeValue = [&](const std::string &ref, FunctionalConstraint fc) -> DataAttribute
        {
            ...
            else
            {
                ...
                case IED_ERROR_OBJECT_ACCESS_UNSUPPORTED:
                    ...
                    break;
                default:
                    errorMsg = "Unknown error: " + std::to_string(readError);
                }
                attr.value = errorMsg;
